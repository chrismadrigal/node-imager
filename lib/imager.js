...
Imager.prototype = {
    ...
    if (variants.resize) {
      Object.keys(variants.resize).forEach(function (name) {
        var processFiles = function (cb) {
          ...
          var fname = name + filetype;
          self.resizeFile(file, preset, fname, cb);
          self.uploadedFiles.push(fname);
        };
  ...
  resizeFile: function (file, preset, fname, cb) {
    ...
    gm(file['path'])
      ...
      .write(dest, function (err) {
        ...
        self['pushTo' + self.storage](dest, fname, fname, file['type'], cb);
      });
  },
  ...
  resizeAndCropFile: function (file, preset, fname, cb) {
    ...
    gm(file['path'])
      .autoOrient()
      .resize(preset.type.resize.split(',')[0], preset.type.resize.split(',')[1])
      .gravity('Center')
      .crop(preset.type.crop.split(',')[0], preset.type.crop.split(',')[1])
      .write(dest, function (err) {
        if (err) return cb(err);
        self['pushTo' + self.storage](dest, fname, fname, file['type'], cb);
      });
